...
const createMaterial = async (req, res) => {
  const { name, type, supplier, description } = req.body;
...
// Récupère les détails d'un matériau
const getMaterialDetailsById = async (req, res) => {
  try {
    const material = await Material.findById(req.params.id);
    if (!material) {
      return res.status(404).json({ msg: 'Matériau non trouvé' });
    }
    res.render('material_details', { material });
  } catch (err) {
    console.error(err.message);
    res.status(500).send('Erreur du serveur');
  }
